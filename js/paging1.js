...
function renderProduct() {
    ...
    const Product = productList.map((item, idx) => {
        if (idx >= start && idx < end) {
            ...
            html += '<div class="box">';
            html += "<a>";
            html += `<div class="img_pr" data-idx="${idx}" id="${idx}"><img src="` + item.image + '"></div>';
...
function show_detail() {
    ...
    const add = productDetail.querySelector(".cart");
    add.addEventListener("click", () => {

        var existLocal = JSON.parse(localStorage.getItem('mainProduct'));
        if (existLocal == null) existLocal = [];
        // var nodeHinhAnh = document.getElementById("main-image").src;
        // var nodeTen = document.getElementById("ten").innerText;
        // var nodeTien = document.getElementById("cost").innerText;
        ...
        //localStorage.setItem('test',JSON.stringify(sanPham));
...
//     return d;
// })
// document.querySelector('.img_pr').addEventListener('click',function (ev) {
//     let index = ev.target.id;
