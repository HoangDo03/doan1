...
function validateInput() {
    ...
    for (let i = 0; i < inputElement.length; i++) {
        if (inputElement[i].value === "") {
            inputElement[i].parentElement.querySelector(".error_message").innerText = `Please enter your ${inputElement[i].id}`;
        }
        else {
            inputElement[i].parentElement.querySelector(".error_message").innerText = "";
        }
    }
}
...
function showProductList() {
    ...
    for (let i = 0; i < productArr.length; i++) {
        table +=
            ...
            '<tr><td>' + (i + 1) + '</td><td>' + productArr[i].title + '</td><td><img src="' + productArr[i].image + '" width=100px , height=130px></td><td>' + productArr[i].category + '</td> <td>' + productArr[i].cost + '</td><th><button onclick="deleteItem(\'' + productArr[i].id + '\')"><i class="fa-solid fa-delete-left"></i><button onclick="editItem(\'' + productArr[i].id + '\')"><i class="fa-solid fa-pen-to-square"></i></th></tr>'

    }
    document.getElementById('table-1').innerHTML = table;
}
...
function chooseFile(fileInput){
    ...
    //kiem tra xem nguoi dung co lua chon file khong
    if(fileInput.files && fileInput.files[0]){
        var reader = new FileReader();

        //doc va hien thi du lieu len the #preview
        reader.addEventListener("load", function listen(e) {
...
function add() {
    ...
    let formElement = document.querySelector("form");
    let errorElement = formElement.querySelectorAll(".error_message");
    let arrError = [];
...
function createNewuser() {
    userArray = JSON.parse(localStorage.getItem('userArr'));
    if( document.getElementById('idselect').value == "admin" ){
    user0 = {
